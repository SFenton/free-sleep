...
import { MqttSettings, MqttSettingsSchema, MqttSettingsUpdateSchema } from '../../db/mqttSettingsSchema.js';
import { normalizeMqttSettings, reloadMqttServiceSettings } from '../../mqtt/mqttService.js';
import logger from '../../logger.js';

const router = express.Router();

export async function updateMqttSettings(mqttSettingsUpdate: Partial<MqttSettings>) {
...
router.get('/mqttSettings', async (_req: Request, res: Response) => {
  await mqttSettingsDB.read();
  res.json(mqttSettingsDB.data);
});

router.post('/mqttSettings', async (req: Request, res: Response) => {
  try {
    const data = await updateMqttSettings(req.body as Partial<MqttSettings>);
