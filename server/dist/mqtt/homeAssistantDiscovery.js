...
const temperatureLevelNumberPayload = (name, stateTopic, commandTopic) => ({
    name,
...
export function buildHomeAssistantDiscoveryMessages({ deviceId, topicPrefix, discoveryPrefix, deviceStatus, settings, }) {
    ...
    for (const side of SideSchema.options) {
        ...
        add('number', `${sideId}_target_temperature`, {
            ...temperatureLevelNumberPayload(`${name} Target Level`, topic(topicPrefix, `${side}/targetTemperatureF/state`), topic(topicPrefix, `${side}/temperature/set`)),
        });
        for (const stage of SCHEDULE_TEMPERATURE_STAGE_KEYS) {
            add('number', `${sideId}_${stage}_temperature`, {
                ...temperatureLevelNumberPayload(`${name} ${SCHEDULE_TEMPERATURE_STAGE_LABELS[stage]}`, topic(topicPrefix, `${side}/schedule/${stage}TemperatureF/state`), topic(topicPrefix, `${side}/schedule/${stage}TemperatureF/set`)),
            });
        }
