...
Web edits to Free Sleep settings, schedules, and services write the LowDB files first. The MQTT client watches those files and republishes the latest retained state to Home Assistant immediately after the write, rather than waiting for the next full refresh interval.
